...
class TweetScraper(CrawlSpider):
    ...
    def parse_tweet_item(self, items):
        for item in items:
            try:
                ...
                ### get text content
                FList = item.xpath('.//div[@class="js-tweet-text-container"]/p//text()').extract()
                LLinks = item.xpath('.//div[@class="js-tweet-text-container"]/p/a[@class="twitter-timeline-link"]//text()').extract()
                LLinks1 = item.xpath('.//div[@class="js-tweet-text-container"]/p/a[@class="twitter-timeline-link u-hidden"]//text()').extract()
                TList = []
                for x in FList:
                    if x not in LLinks+LLinks1: 
                        TList.append(x)
                tweet['text'] = ' '.join(TList).replace(' # ','#').replace(' @ ', '@')
    ...
    def extract_one(self, selector, xpath, default=None):
        ...
        return default
"""
Created on Fri Nov 30 03:44:37 2018
